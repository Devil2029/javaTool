...
public class HttpClientUtils {
	...
	static {
		// 初始化线程池
		RequestConfig params = RequestConfig.custom().setConnectTimeout(3000)
		...
		HttpRequestRetryHandler retryHandler = new HttpRequestRetryHandler() {
			public boolean retryRequest(IOException exception,
				...
				if (exception instanceof NoHttpResponseException) {
					LOGGER.error("[NoHttpResponseException has retry request:"
							+ context.toString() + "][executionCount:"
							+ executionCount + "]");
					return true;
				} else if (exception instanceof SocketException) {
					LOGGER.error("[SocketException has retry request:"
							+ context.toString() + "][executionCount:"
							+ executionCount + "]");
					return true;
				}
		...
		httpclient = HttpClients.custom().setConnectionManager(pccm)
				.setDefaultRequestConfig(params).setRetryHandler(retryHandler)
				.build();
	}

	public static String post(String urlToRequest,
			Map<String, Object> parameters, Integer connectionRequestTimeout,
		...
		try {
			LOGGER.error("post-req:url:{},param:{}", urlToRequest,
					JSON.toJSONString(parameters));
			HttpPost post = new HttpPost(urlToRequest);

			if (connectionRequestTimeout != null && connectTimeout != null
					&& socketTimeout != null) {
			...
			if (currentMethodCallTime > 5000) {
				LOGGER.error("url:{},call time {} ms", urlToRequest,
						currentMethodCallTime);
				LOGGER.error("所有存活线程=" + Thread.getAllStackTraces().size());
			}
			LOGGER.error("post-rps:{}", result);
			return result;
		} catch (UnsupportedEncodingException e) {
			LOGGER.error(
					"[HttpClientUtils][post][Unsupported Encoding Exception]",
					e);
		}
		return null;
	}
